...
fun main() {
  ...
  File("p14.txt").forEachLine {
    if (it == "") {
      ...
      for (i in 0..it.length-2) {
        ...
        pairs[p] = pairs.getOrDefault(p, 0L) + 1L
      }
  ...
  for (step in 0..39) {
    var newPairs = mutableMapOf<String, Long>()
    rules.forEach { k, v ->
      if (!pairs.contains(k)) return@forEach
      newPairs[k[0] + v] = newPairs.getOrDefault(k[0] + v, 0L) + pairs[k]!!
      newPairs[v + k[1]] = newPairs.getOrDefault(v + k[1], 0L) + pairs[k]!!
    }
    pairs = newPairs
    println("${pairs.values.sum()} pairs after step $step")
  }

  var elems = mutableMapOf<Char, Long>(lastChar to 1)
  pairs.forEach { k, v ->
    elems[k[0]] = elems.getOrDefault(k[0], 0L) + v
  }
